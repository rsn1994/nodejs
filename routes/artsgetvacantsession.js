...
           path: '/arts/get/filled/{code}/{date}',

          handler: function (request, reply) {
var colcde=encodeURIComponent(request.params.code);
var indate=encodeURIComponent(request.params.date);           
    db.connection.query('SELECT * FROM artsfull WHERE colCode=? AND idate=? ORDER BY id ASC',[colcde,indate], function(err, rows,   fields) {
  if (err) throw err;
var sts= new Array();
sts.push({
...
   name    :'user-artsgetfilledbysession',
